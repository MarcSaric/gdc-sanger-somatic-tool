...
from gdc_sanger_tools.subcommands.base import Subcommand
from gdc_sanger_tools.utils import AlignmentHeaderT, ArgParserT, LoggerT, NamespaceT
...
class CheckBamHeader(Subcommand):
    ...
    def __add_arguments__(cls, parser: ArgParserT):
        ...
        parser.add_argument(
            "--input_bam", required=True, help="Input BAM file to check."
        )
        parser.add_argument(
            '--aliquot_id',
            required=True,
    ...
    def check_header(cls, header: AlignmentHeaderT, logger: LoggerT) -> bool:
        ...
        if not new_header and len(set(samples)) != 1:
            logger.warning(
                "Multiple sample IDs detected {0}".format(list(set(samples)))
            )
            new_header = True
    ...
    def build_new_header(
        cls, header: AlignmentHeaderT, aliquot_id: str
    ) -> Dict[str, Any]:
        """
    ...
    @classmethod
    def main(cls, options: NamespaceT, logger=logger, di=DI) -> None:
        """
        ...
        # setup
        bam = di.pysam.AlignmentFile(options.input_bam, mode="rb")

        try:
            if cls.check_header(bam.header, logger):
                logger.info(
                    "Detected RG problems, will create new header with SM {0}".format(
                        options.aliquot_id
                    )
                )
                # build new header
                new_header = cls.build_new_header(bam.header, options.aliquot_id)
                obam = di.pysam.AlignmentFile(
                    options.output_header, mode="w", header=new_header
                )
                obam.close()
            else:
    ...
    def __get_description__(cls):
        ...
        """
        return (
            "Checks the bam header to make sure all rgs have "
